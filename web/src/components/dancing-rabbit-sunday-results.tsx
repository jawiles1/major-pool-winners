...
export function DancingRabbitSundayResults({ result }: { result: DailyResult }) {
  ...
  return (
    <section id="sunday-results" aria-labelledby="sunday-results-title" className="rounded-[1.5rem] border border-line bg-card/90 p-5">
      ...
      <p className="mt-2 text-sm text-muted">{result.complete ? "All 18 holes complete." : "Round in progress — results and payouts are not final."} Each foursome counts one lowest gross and one lowest net score per hole, totaled over 18 holes. The same player may count for both.</p>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">{(["gross", "net"] as const).map(ball => {
        ...
        return <article key={ball} className="rounded-xl border border-line bg-background/70 p-4">
          <h3 className="text-lg font-semibold">Low {ball} · {formatMoney(result.day.stakePerPlayer ?? 0)}/player</h3>
      ...
      <p className="mt-3 text-sm text-muted">Gross and net bets only. Eagle bounties and the overall trip pool are separate. Payment splits between recipients do not change each player’s total.</p>
    </section>
  );
}
